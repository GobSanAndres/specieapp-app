...
const FormScreen = props => {
  ...
  const {formJson, jsonRequired} = getKeys(survey); //jsonRequired es un json con el valor de required de cada una de las preguntas (OJO aplicar cambios a getKeys)
  ...
  const {idTheme, isNetwork, localSurveyFish, user, localSurveyAgro, idIntern} =
  ...
  const sytleNetwork = {
    backgroundColor: isNetwork
      ? theme.primary.button.backgroundColor
      : theme.primary.button.secondBgColor,
  };
  ...
  const handleSend = () => {
    ...
    if (validDataJson(formState, jsonRequired)) {
      //Se agrega jsonRequired validDataJson (Ojo modificar función validDataJson)
      ...
      AlertGeneric(
        'Advertencia',
        'Debe completar todos los campos marcados como obligatorios',
        'OK',
        true,
        null,
      );
      ...
      .then(resp => {
        ...
        if (resp.success) {
          if (formState.idintern) {
            ...
            arrayLocal = localData.filter(
              item => item.idintern != formState.idintern,
            );
            if (origin == 'fish')
              dispatch(setLocalSurveyFishMiddleware(arrayLocal));
            else dispatch(setLocalSurveyAgroMiddleware(arrayLocal));
          }
          ...
          AlertGeneric(
            'Error',
            '¡Ups ha sucedido un error, intente nuevamente en unos minutos!',
            'OK',
            true,
            null,
          );
  ...
  const saveLocal = () => {
    ...
    dispatch(setIsLoadingMiddleware(true));
    if (origin == 'fish') dispatch(setLocalSurveyFishMiddleware(arrayLocal));
    else dispatch(setLocalSurveyAgroMiddleware(arrayLocal));
      ...
      {!isOpenAcordion && (
        <View style={styles.containButton}>
          <ButtonCustom
            ...
            color={
              isNetwork
                ? theme.primary.button.textColor
                : theme.primary.button.secondeTxColor
            }
            styleButton={{
              ...styles.button,
              ...sytleNetwork,
