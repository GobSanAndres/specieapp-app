...
const AccordionCustom = props => {
  const {encuestas, formState, setFormState, setIsOpenAcordion, intentSend} =
    props;
  ...
  const [duplicSection, setDuplicSection] = useState(
    getDuplicateSection(sections, formState),
  );
  ...
  const itereactDuplicate = () => {
    ...
    if (countArrayDuplicate.length > 0) {
      handleSection(
        countArrayDuplicate[0].name,
        countArrayDuplicate[0].duplicate - 1,
      );
    }
  };
  ...
  const handleSection = (section, interacte) => {
    ...
    encuestas.sections.map(item => {
      if (section.idSection != item.idSection)
        newSections.push(
          sections.filter(sec => sec.idSection == item.idSection)[0],
        );
      else {
        let itemQuestion = [];
        for (let index = 0; index < interacte; index++) {
          item.question.map(element => {
            const quantity = duplicSection.filter(
              dup => dup.id == section.idSection,
            )[0];
        ...
        const copyObject = copy.filter(cop => cop.id == section.idSection)[0]
          .value;
  ...
  const updateDuplicate = (section, increment) => {
    let restDup = [];
    const changeDup = duplicSection.filter(
      dup => dup.id == section.idSection,
    )[0];
    restDup = duplicSection.filter(dup => dup.id != section.idSection);
    restDup.push({
      id: changeDup.id,
      duplicate: changeDup.duplicate + increment,
    });
  ...
  const handleDelete = section => {
    ...
    let newForms = {};
    sections.map(item => {
      if (section.idSection != item.idSection) newSections.push(item);
      else {
        let question = [];
        ...
        for (
          let index = 0;
          index < formState[section.nameQuestion].length - 1;
          index++
        ) {
            ...
            return (
              <List.Accordion
                ...
                left={props => (
                  <List.Icon
                    {...props}
                    color={theme[idTheme].accordion.color}
                    icon="folder"
                  />
                )}
                      ...
                      case 'input':
                        return (
                          <Input
                            ...
                            index={i}
                            isMultiline={
                              item.widget_type == 'textarea' ? true : false
                            }
                          />
                        );
